...
class FuzzyRec:
    def __init__(self,
        ...
        self.user_num = max(users_dict.keys())
        self.item_num = max(users_dict.keys())
    ...
    def load_all_embeddings(self, metapaths, node_num, node_type='user'):
        ...
        for metapath in metapaths:
            metapath_emb_file_path = os.path.join(self.embeddings_dir_path, '{}_embs.txt'.format(metapath))
            with open(metapath_emb_file_path, 'r', encoding='utf-8') as infile:

                k = int(str(infile.readline().strip().split(' ')[1]))
                ...
                for line in infile.readlines():
                    ...
                    idx = str(arr[0])
                    if not idx.startswith('prod_') and not idx.startswith('user_'):
                        continue
                    idx = idx.replace('prod_', '').replace('user_', '')
                    i = int(idx) - 1
                    if i in X:
                        for j in range(k):
                            if j in X[i][acc]:
        ...
        if node_type == 'user':
            sem_emb_file_path = os.path.join(self.embeddings_dir_path, 'user_fuzzy_embs.txt')
        else:
            sem_emb_file_path = os.path.join(self.embeddings_dir_path, 'item_fuzzy_embs.txt')
    ...
    def cal_sem_v(self, j):
        ...
        vj_sem += self.p_vsem[j] * self.sigmod_func((self.W_vsem.dot(self.Y[j][self.item_metapath_num]) + self.b_vsem))
            ...
            for t in self.R:
                ...
                for k in range(self.user_metapath_num):
                    if k in self.Wu:
                        ...
                        bu_g = self.lambda_constant * -eij * self.pu[i][k] * self.H[j, :] + self.beta_hyper_param * \
                               self.bu[
                                   k]
